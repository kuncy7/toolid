...
def update_tool(tool_id: int, data: ToolUpdate, session: Session = Depends(get_session)):
    ...
    if 'quantity_total' in update_data:
        ...
        if new_total < loaned_count:
            raise HTTPException(400, f"Cannot set total quantity to {new_total}, because {loaned_count} items are currently on loan.")
...
def upload_base64_image(tool_id: int, payload: Base64ImagePayload, session: Session = Depends(get_session)):
    ...
    try:
        ...
        if not match: raise HTTPException(status_code=400, detail="Invalid image format. Only jpeg, png, gif are supported.")
...
def create_loan(tool_id: int, payload: LoanCreate, session: Session = Depends(get_session)):
    tool = session.get(Tool, tool_id)
    if not tool: raise HTTPException(404, "Tool not found")
    if not session.get(User, payload.user_id): raise HTTPException(404, "User not found")
    if tool.quantity_available <= 0:
        raise HTTPException(status_code=400, detail="No available items for this tool.")
    tool.quantity_available -= 1
    session.add(tool)
    loan = ToolLoan(tool_id=tool_id, user_id=payload.user_id)
    session.add(loan)
    session.commit()
...
def return_tool_loan(loan_id: int, session: Session = Depends(get_session)):
    ...
    if loan.returned: raise HTTPException(400, "Tool has already been returned")
...
def add_weight_measurement(tool_id: int, payload: WeightCreate, session: Session = Depends(get_session), current_user: dict = Depends(get_current_user)):
    if not session.get(Tool, tool_id): raise HTTPException(404, "Tool not found")
    measurement = ToolWeight(tool_id=tool_id, weight_value=payload.weight_value, measured_by=current_user.get("sub"))
    session.add(measurement); session.commit(); session.refresh(measurement)
    return measurement
