...
def update_config(
    payload: WarehouseConfigPayload, session: Session = Depends(get_session)
):
...
def create_mapping(payload: ToolMappingCreate, session: Session = Depends(get_session)):
    ...
    existing = session.exec(
        select(ToolMapping).where(
            ToolMapping.external_tool_id == payload.external_tool_id
        )
    ).first()
    if existing:
        raise HTTPException(
            400, f"External tool ID '{payload.external_tool_id}' is already mapped."
        )
