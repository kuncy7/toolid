...
def create_integration(payload: IntegrationCreate, session: Session = Depends(get_session)):
...
def update_integration(integration_id: str, payload: IntegrationUpdate, session: Session = Depends(get_session)):
    integration = session.get(ExternalIntegration, integration_id)
    if not integration: raise HTTPException(404, "Integration not found")
...
def logs_integration(integration_id: str, session: Session = Depends(get_session)):
    if not session.get(ExternalIntegration, integration_id): raise HTTPException(404, "Integration not found")
    return session.exec(select(IntegrationLog).where(IntegrationLog.integration_id == integration_id)).all()
