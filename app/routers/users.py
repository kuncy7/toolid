...
@router.get("", response_model=List[User], dependencies=[Depends(require_role("admin"))])
...
@router.get("/{user_id}", response_model=User, dependencies=[Depends(require_role("admin","moderator"))])
def get_user(user_id: str, session: Session = Depends(get_session)):
    obj = session.get(User, user_id)
    if not obj: raise HTTPException(404, "User not found")
    return obj

@router.put("/{user_id}", response_model=User, dependencies=[Depends(require_role("admin"))])
...
@router.post("/{user_id}/reset-password", status_code=204, dependencies=[Depends(require_role("admin"))])
def reset_password(user_id: str, payload: PasswordReset, session: Session = Depends(get_session)):
...
@router.get("/{user_id}/permissions", response_model=List[UserPermission], dependencies=[Depends(require_role("admin"))])
def get_user_permissions(user_id: str, session: Session = Depends(get_session)):
    ...
    return session.exec(select(UserPermission).where(UserPermission.user_id == user_id)).all()

@router.put("/{user_id}/permissions", response_model=UserPermission, dependencies=[Depends(require_role("admin"))])
def update_user_permission(user_id: str, payload: PermissionUpdate, session: Session = Depends(get_session)):
    if not session.get(User, user_id): raise HTTPException(404, "User not found")
    ...
    session.add(permission); session.commit(); session.refresh(permission)
    return permission
